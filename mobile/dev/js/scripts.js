    ...
    // Scripts Pagina de Produto //
	    if (pagProduto.length > 0) {
			try {
				...
				$('.Cor').addClass('col-xs-6');
				
				if($('.Cor label').length > 3){
					$('.Cor').removeClass('col-xs-6');
					$('.Cor').addClass('col-xs-12');
				}
			...
			try {
				$document.ajaxStop(function() {
					...
					if($('.selecao-sku label').length < 2){
						$('.topic.Cor').remove();
						$('.selecao-sku .quantidade').removeClass('col-xs-6');
						$('.selecao-sku .quantidade').addClass('qtd-50');
					};
				});
			} catch(e) {}
						...
						$.each(data, function(key, val) {
							...
							$(elements).each(function(data, val){
								// Take Image Thumbs //
									...
									$('<li class="slick-slide '+myLabel+'"><img src="/arquivos/ids/'+myImageID+'-600-600/'+myImageName+'.jpg" /></li>').appendTo('.ProductImage .slider ul');
								// Take Image Thumbs //
							});
